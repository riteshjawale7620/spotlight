...
import { LeaderItem } from '@/lib/data/mockData'

interface LeadersContentProps {
  initialLeaders: LeaderItem[]
}

export default function LeadersContent({ initialLeaders }: LeadersContentProps) {
  ...
  // Dynamic filter over leaders
  const filteredLeaders = initialLeaders.filter((leader) => {
    if (searchQuery) {
      const q = searchQuery.toLowerCase()
      const matchName = leader.name.toLowerCase().includes(q)
      const matchRole = leader.role.toLowerCase().includes(q)
      const matchBio = leader.bio.toLowerCase().includes(q)
      const matchOrg = (leader.organization || '').toLowerCase().includes(q)
      if (!matchName && !matchRole && !matchBio && !matchOrg) return false
    }
    if (selectedIndustry !== 'All Industries') {
      const ind = selectedIndustry.toLowerCase()
      const matchBadge = leader.badge.toLowerCase().includes(ind)
      const matchBio = leader.bio.toLowerCase().includes(ind)
      if (!matchBadge && !matchBio) return false
    }
    return true
  })
  ...
  const spotlightStories = [
    {
      date: 'MAY 2024 EDITION',
      title: 'The Industrial Vanguard: Inside Epic Group’s Decarbonized Ecosystem',
    ...
    {
      date: 'AUG 2026 EDITION',
      title: 'Soil Microbiome Intelligence & Climate Resilience in Agriculture',
      category: 'AGRIBIO',
    ...
    {
      date: 'SEP 2024 EDITION',
      title: 'Community-First Capitalism: Navigating Volatility with Enduring Trust',
      category: 'FINANCE',
      readingTime: '6 MIN READ',
    ...
    {
      date: 'JUL 2024 EDITION',
      title: 'Reprogramming Cellular Defense: Next-Generation Immunotherapies',
      category: 'HEALTHCARE',
    ...
    {
      date: 'JAN 2026 EDITION',
      title: 'Molecular Innovation: Eliminating Plastics Through Bio-Assimilable Resins',
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1">
          ...
          <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center">
              ...
              <div className="lg:col-span-5 flex justify-center lg:justify-end">
                <div className="relative w-full max-w-[420px] aspect-[4/5] overflow-hidden rounded-[2px] shadow-xl border border-neutral-200 bg-neutral-200 group">
                  <Image
                    src={initialLeaders[0]?.imageUrl || '/images/leaders-hero-portrait.jpg'}
                  ...
                  <div className="absolute bottom-0 inset-x-0 p-5 sm:p-6 text-white flex flex-col justify-end">
                    ...
                    <h3 className="font-serif text-lg text-white font-normal leading-snug">
                      {initialLeaders[0]?.name || 'Architects of Sustainable Enterprise'}
                    </h3>
                    <span className="text-[10px] uppercase tracking-wider text-white/60 font-sans mt-1">
                      {initialLeaders[0]?.role || 'Executive Series • Global Edition'}
                    </span>
                  </div>
                </div>
              </div>
        ...
        {/* ============================================================ */}
        <section className="w-full bg-neutral-50 border-b border-neutral-200 py-4">
          <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
              ...
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full bg-white border border-neutral-200 px-4 py-2.5 pr-10 text-xs font-sans text-neutral-800 placeholder:text-neutral-400 focus:outline-hidden focus:border-[#A17A38] transition-colors"
                />
        ...
        {/* ============================================================ */}
        {/* SECTION 3: FEATURED LEADERS (Live Sanity Grid)               */}
        {/* ============================================================ */}
        <section id="featured-leaders" className="w-full bg-white border-b border-neutral-200 py-14 lg:py-18">
          <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-4 sm:gap-6 mb-8 sm:mb-10">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-[#A17A38] shrink-0">
                {filteredLeaders.length} Featured Leaders
              </span>
            </div>
            ...
            <div className="relative">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {filteredLeaders.map((leader) => (
                  <div
                    key={leader.id}
                    className="bg-white border border-neutral-200 p-4 flex flex-col justify-between group hover:shadow-md hover:border-[#A17A38]/50 transition-all duration-300"
                  >
                    <div>
                      {/* Portrait */}
                      <Link href={`/leaders/${leader.slug}`} className="block relative w-full aspect-[4/5] overflow-hidden bg-neutral-100 mb-4">
                        <Image
                          src={leader.imageUrl}
                          alt={leader.name}
                          fill
                      ...
                      {/* Name & Title */}
                      <h3 className="font-serif text-base font-bold text-[#141414] leading-snug">
                        <Link href={`/leaders/${leader.slug}`} className="hover:text-[#A17A38] transition-colors">
                          {leader.name}
                        </Link>
                      </h3>
                      <p className="text-xs text-neutral-500 font-sans mt-0.5 leading-snug font-medium">
                        {leader.role}
                      </p>
                      {leader.organization && (
                      ...
                      <div className="mt-2.5 mb-2.5">
                        <span className="inline-block px-2.5 py-0.5 bg-[#A17A38]/10 text-[#A17A38] text-[9.5px] font-sans font-bold tracking-wider uppercase rounded-xs">
                          {leader.badge}
                        </span>
                      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 sm:gap-4">
              {expertiseList.map((item) => {
                ...
                return (
                  <div
                    key={item.title}
                    onClick={() => setSelectedExpertise(item.title)}
                    className="bg-white border border-neutral-200 p-4 text-center flex flex-col items-center justify-between hover:border-[#A17A38] hover:shadow-sm transition-all group cursor-pointer"
                  >
                    <div className="w-10 h-10 rounded-full bg-neutral-100 flex items-center justify-center text-neutral-700 group-hover:text-[#A17A38] group-hover:scale-110 transition-all mb-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
              <div className="lg:col-span-5 space-y-4">
                ...
                <p className="text-sm text-neutral-400 font-sans leading-relaxed pt-1">
                  An exclusive conversation with {initialLeaders[0]?.name || 'Ranjan Mahtani'} on building a
                  purpose-driven global enterprise and engineering sustainable industrial scale.
                </p>

                <p className="text-[11px] uppercase tracking-[0.2em] text-[#C5A059] font-sans font-semibold pt-1">
                  08 MIN READ • EXECUTIVE SERIES
                </p>

                <div className="pt-2">
                  <Link
                    href={`/leaders/${initialLeaders[0]?.slug || 'ranjan-mahtani'}`}
              ...
              <div className="lg:col-span-3 flex items-center justify-center">
                <div className="relative w-full max-w-[280px] aspect-[3/4] overflow-hidden rounded-[2px] shadow-2xl">
                  <Image
                    src={initialLeaders[0]?.imageUrl || '/images/leaders-hero-portrait.jpg'}
                    alt={initialLeaders[0]?.name || 'Leader'}
                    fill
              ...
              <div className="lg:col-span-4 space-y-4 lg:pl-6 border-t lg:border-t-0 lg:border-l border-[#26262B] pt-6 lg:pt-0">
                ...
                <blockquote className="font-serif text-xl sm:text-2xl text-white font-light leading-snug">
                  {initialLeaders[0]?.quote || 'Disruption is not about reckless novelty; it is the courage to reconstruct legacy manufacturing into an eco-conscious, technologically agile ecosystem.'}
                </blockquote>

                <div className="pt-2">
                  <span className="font-serif text-sm text-white font-semibold block">
                    {initialLeaders[0]?.name || 'Ranjan Mahtani'}
                  </span>
                  <span className="text-xs text-[#C5A059] font-sans font-medium block">
                    {initialLeaders[0]?.role || 'Founder & Executive Chairman'} • {initialLeaders[0]?.organization || 'Epic Group'}
                  </span>
                </div>
              </div>
            </div>
        ...
        {/* ============================================================ */}
        <section id="spotlight-leaders" className="w-full bg-white border-b border-neutral-200 py-14 lg:py-18">
          <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-4 sm:gap-6 mb-8 sm:mb-10">
              <h2 className="font-serif text-xl sm:text-2xl uppercase tracking-[0.12em] text-[#121214] font-medium shrink-0">
                Leaders Spotlight
                  ...
                  <div className="p-3 flex flex-col justify-between flex-1">
                    <div>
                      ...
                      <h3 className="font-serif text-sm font-semibold text-[#141414] leading-snug group-hover:text-[#A17A38] transition-colors">
                        <Link href={`/leaders/${story.slug}`}>{story.title}</Link>
                      </h3>
                    </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-5">
              {industriesList.map((ind) => (
                <Link
                  ...
                  className="relative h-36 sm:h-40 overflow-hidden rounded-[2px] border border-neutral-200 group flex flex-col justify-end p-4 text-white shadow-xs"
                >
                  <Image
                    src={ind.imageUrl}
                    alt={ind.name}
        ...
        {/* ============================================================ */}
        <section className="w-full bg-neutral-50 border-b border-neutral-200 py-14 lg:py-18">
          <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center">
                  ...
                  <form onSubmit={handleSubscribe} className="space-y-3 pt-2">
                    <div className="flex flex-col sm:flex-row items-stretch gap-2 max-w-lg">
                      <input
                        ...
                        className="flex-1 bg-white border border-neutral-200 px-4 py-3 text-xs font-sans text-neutral-800 placeholder:text-neutral-400 focus:outline-hidden focus:border-[#A17A38]"
                      />
                      <button
                        type="submit"
